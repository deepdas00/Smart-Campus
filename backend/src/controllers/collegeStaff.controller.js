...
export const staffLogin = asyncHandler(async (req, res) => {
    ...
    if(!collegeCode ||!loginId || !password){
      return res.status(400).json({ message: "Missing required fields: college, loginId, or password" });
    }
    // 1️⃣ Resolve college
...
export const currentStaff = asyncHandler(async (req, res) => {
  try {
    ...
    const currentStaff = await Staff.findById(userId).select("-password -refreshToken");
