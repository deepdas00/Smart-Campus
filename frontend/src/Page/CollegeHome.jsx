...
import {
  ...
  Calendar, Hash, Sparkles, ChevronRight, Info, Globe, Bell, FileText, X
} from "lucide-react";
...
export default function SmartCollegeAdmin() {
  ...
  const InputGroup = ({ label, icon, value, onChange }) => (
    <div className="flex flex-col gap-2">
      <label className="text-[10px] font-black uppercase text-blue-600 tracking-widest">{label}</label>
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-white p-3 pl-12 rounded-2xl border-2 border-slate-100 focus:border-blue-500 outline-none text-sm font-bold shadow-sm text-slate-700"
        />
      </div>
    </div>
  );
  ...
  const deleteDepartment = (indexToDelete) => {
    setCollegeData({
      ...collegeData,
      departments: collegeData.departments.filter((_, index) => index !== indexToDelete)
    });
  };
        ...
        <div className="flex gap-4 mb-8">
          {[
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-3 p-5 rounded-2xl font-bold transition-all border-2 ${activeTab === tab.id
          ...
          <div className="grid lg:grid-cols-12 gap-6 animate-fadeIn">
            <div className="lg:col-span-8 bg-[#bbcbff] p-8 rounded-[2.5rem] border border-blue-100 shadow-inner">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <InputGroup label="Personal Contact Name" icon={<UserIcon size={20} />} value={collegeData.personalContactName} onChange={(v) => setCollegeData({ ...collegeData, personalContactName: v })} />
                <InputGroup label="Registration Number" icon={<FileText size={20} />} value={collegeData.regNumber} onChange={(v) => setCollegeData({ ...collegeData, regNumber: v })} />
                <InputGroup label="Contact Number" icon={<Phone size={20} />} value={collegeData.officePhone} onChange={(v) => setCollegeData({ ...collegeData, officePhone: v })} />
              </div>

              {/* Departments Section */}
              <div className="mt-8">
                <label className="text-xs font-black uppercase text-blue-600 mb-3 block tracking-widest">Departments</label>
