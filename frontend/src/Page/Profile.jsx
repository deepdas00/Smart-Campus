...
export default function App() {
    ...
    libraryHistory: [
      {
        ...
        returned: "Oct 12, 2024",
        author: "Andrew Ng",
      },
      {
        id: "H2",
        title: "Organic Chemistry",
        returned: "Sept 05, 2024",
        author: "J. Smith",
      },
    ],
    campusComplaints: [
      {
        id: "C-1",
        subject: "WiFi Deadzone - Library Floor 2",
        status: "Under Review",
        date: "21 Dec",
      },
      {
        id: "C-2",
        subject: "Broken Fan - Room 402",
        status: "Resolved",
        date: "18 Dec",
      },
    ],
  ...
  const LibraryPage = () => (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500 mt-[-35px]">
      <div>
        <h3 className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.3em] mb-4">
          Currently Issued
        </h3>
        <div
          className={`p-8 rounded-[2.5rem] bg-gradient-to-br ${userData.activeIssue.color} text-white shadow-2xl flex flex-col md:flex-row justify-between items-center gap-6`}
        >
          <div className="flex items-center gap-6">
            <div className="w-20 h-28 bg-white/20 backdrop-blur-md border border-white/30 rounded-xl flex items-center justify-center">
              <BookOpen size={40} />
            </div>
            <div>
              <h2 className="text-3xl font-black tracking-tighter italic">
                {userData.activeIssue.title}
              </h2>
              <p className="opacity-80 font-bold uppercase text-[10px] tracking-widest mt-1">
                Return by {userData.activeIssue.due}
              </p>
            </div>
          </div>
      ...
      <div>
        ...
        <div className="grid gap-3">
          {userData.libraryHistory.map((item) => (
            <div
              key={item.id}
              className="p-5 bg-white border border-slate-100 rounded-3xl flex justify-between items-center group hover:border-slate-300 transition-all"
            >
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {userData.canteenOrders.map((order) => (
          <div
            key={order.id}
            className="p-6 bg-white rounded-[2rem] border border-slate-100 shadow-sm flex justify-between items-center"
          >
            <div className="flex items-center gap-4">
      ...
      <div className="space-y-3">
        {userData.campusComplaints.map((item) => (
          ...
          >
            ...
            <div
              className={`text-[9px] font-black uppercase tracking-[0.2em] px-3 py-1.5 rounded-lg ${
      ...
      <div className="h-screen w-screen bg-[#fcfcfd] flex overflow-hidden font-sans text-slate-900">
        ...
        <aside className="w-20 lg:w-64 flex flex-col border-r border-slate-100 bg-white">
          <div className="p-8 flex items-center justify-center lg:justify-start gap-3">
            ...
            <span className="hidden lg:block font-black text-xl tracking-tighter italic">
              HUB.
            </span>
          </div>
            ...
            {activeTab === "Overview" ? (
              <div className="space-y-10">
                <section>
                  ...
                  <div
                    className={`p-6 rounded-[2rem] bg-gradient-to-r ${userData.activeIssue.color} text-white flex justify-between items-center`}
                  >
                    <div className="flex items-center gap-4">
                      <BookOpen size={24} />
                    ...
                    <button
                      onClick={() => setActiveTab("Library")}
                      className="bg-white/20 hover:bg-white/30 backdrop-blur-md px-4 py-2 rounded-xl text-[10px] font-black uppercase"
                    >
                      View History
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                    ...
                    <div className="flex justify-between items-center font-bold text-sm">
                      ...
                      <span className="text-orange-500 tracking-tighter italic">
                        {userData.canteenOrders[0].status}
                      </span>
                    </div>
                  </div>
                  <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
