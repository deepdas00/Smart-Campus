              ...
              {loading ? (
                ...
                filteredStudents.map((s) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black tracking-wide ${s.isActive
                          ? "bg-emerald-100 text-emerald-700"
                          : "bg-rose-100 text-rose-700"
                          }`}
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${s.isActive ? "bg-emerald-500" : "bg-rose-500"
                            }`}
                        />
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="pt-2 pb-10">
            <button
              ...
              className={`w-full py-4 rounded-[1.5rem] font-black uppercase tracking-widest text-xs shadow-2xl transition-all active:scale-[0.96] flex items-center justify-center gap-3 ${isFormValid
...
function StudentDetailsDrawer({ isOpen, onClose, data, loading }) {
  ...
  return (
    <>
      ...
      {/* Slide-over */}
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-xl bg-white shadow-2xl z-[250] transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full"
          } overflow-y-auto`}
      >
        {/* Header */}
        <div
                    ...
                    <div className="space-y-3">
                      {data.libraryTransaction.map((tr) => {
                        ...
                        return (
                          <div
                            key={tr._id}
                            className={`relative rounded-2xl p-4 border transition-all duration-300 flex gap-4 ${hasFine
